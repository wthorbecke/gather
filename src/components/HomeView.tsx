  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-[540px] mx-auto">
        ...
        {/* Coach Whisper - subtle inline insight, appears below input after completions */}
        {!isQuestionFlow && !aiCard && (
          <CoachWhisper />
        )}
        ...
        {(() => {
          ...
          // Progressive disclosure: show only 5 tasks initially
          ...
          return (
            <div>
              ...
              <div className="flex flex-col gap-2">
                {/* Initially visible incomplete tasks (first 5) */}
                {incompleteTasks.slice(0, visibleLimit).map((task, index) => {
                  // Find the first incomplete step for quick complete
                  const firstIncompleteStep = task.steps?.find(s => !s.done)
                  return (
                    <div
                      key={task.id}
                    ...
                    >
                      <TaskListItem
                        ...
                        onQuickComplete={firstIncompleteStep ? () => onToggleStep(task.id, firstIncompleteStep.id) : undefined}
                      />
                    </div>
                  )
                })}

                {/* Expandable hidden tasks (beyond first 5) with smooth height animation */}
                {hiddenCount > 0 && (
                  <div className={`expand-container ${showAllTasks ? 'expanded' : ''}`}>
                    <div className="expand-content">
                      <div className="flex flex-col gap-2">
                        {incompleteTasks.slice(visibleLimit).map((task, index) => {
                          const firstIncompleteStep = task.steps?.find(s => !s.done)
                          return (
                            <div
                              key={task.id}
                              className={showAllTasks ? 'animate-rise' : ''}
                              style={{ animationDelay: showAllTasks ? `${index * 40}ms` : '0ms' }}
                            >
                              <TaskListItem
                                task={task}
                                onClick={() => onGoToTask(task.id)}
                ...
                {hiddenCount > 0 && (
                  <button
                    ...
                    className="
                      w-full py-2.5 mt-1
                      text-xs text-text-muted hover:text-text-soft
                  ...
                  >
                    ...
                    {showAllTasks ? 'Show less' : `Show ${hiddenCount} more task${hiddenCount !== 1 ? 's' : ''}`}
                  </button>
                )}
                ...
                {completedTasks.length > 0 && !showCompletedTasks && (
                  <button
                    ...
                    className="
                      py-3 px-4
                      bg-success-soft rounded-md
                      text-sm text-text-soft
                      hover:bg-success-soft/80 transition-colors duration-150
